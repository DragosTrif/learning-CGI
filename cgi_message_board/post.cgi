...
if ($query->param('action')) {
	...
	if ($action eq 'new') {
		# creating an error message with the 
		# validate_form();
...
sub validate_form {
	...
	if ($title eq "" && $action eq "new") {
		$error_message .= "<li>You must have a title </li>\n";
	}
	if ($author eq "") {
		$error_message .= "<li>You must enter an author.</li>\n";
	}
	if ($post eq "") {
		$error_message .= "<li>You enter a message.</li>\n";
	}
	if ($error_message eq "") {
		return undef;
	}else {	
...
sub write_new_topic {
	...
	# by appending the output of the time function
	chdir;
	my $topic = time . $$;
	open (TOPIC, '>', "$topic_directory/$topic.xml") or die "Cant open '$topic_directory/$topic.xml' $!:";
	print TOPIC "<topic>\n";
...
sub print_page_start {
	...
	print "<body><h1>$page_title</h1>\n";
}

sub print_sucess {
	print "<p><b>Created a new topic: $title.</b></p>\n";
}
...
sub print_error {
	...
	print "<p>Please corect the following errors</p>\n";
...
sub write_response {
	...
	chdir '/tmp';
	print "|$topic_file |\n";
	open (TOPIC, '+<', $topic_file) or die "Can’t open '$topic_directory/$topic_file' $!:";
	flock TOPIC, 2;
	seek TOPIC, 0, 0;
	my @topic_text = <TOPIC>;
